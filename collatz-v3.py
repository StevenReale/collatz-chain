...
def collatz(start_num, chain_dict):
    ...
    #passes next number to test into this function; checks returned dictionary for the chain length of next number on chain; adds 1 to that value and stores it as the chain length for the current number; returns updated dictionary
    newVal = collatz(nextCol, chain_dict)
    chain_dict[start_num] = newVal[nextCol] + 1
    return chain_dict

#loop populates dictionary with keys from 1 to testMax and captures entry with longest chain
